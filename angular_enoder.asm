...
encoder:
; up,down	if button=up   then increment/decrement up	 
; up,down	if button=down then incremnt/decrement down 
...
i_up:	
	...
	rjmp	a_rise
a_fall:
	inc	up					; if B=1 then increment
	sbrs	_w,ENCOD_B
	subi	up,2			; if B=0 then decrement
	rjmp	i_up_done
a_rise:
	inc	up					; if B=0 then increment
	sbrc	_w,ENCOD_B
	subi	up,2			; if B=1 then decrement
i_up_done:
	clz						; clear Z
	ret
...
i_down:	
	...
	rjmp	a_rise2
a_fall2:
	inc	down					; if B=1 then increment
	sbrs	_w,ENCOD_B
	subi	down,2			; if B=0 then decrement
	rjmp	i_down_done
a_rise2:
	inc	down					; if B=0 then increment
	sbrc	_w,ENCOD_B
	subi	down,2			; if B=1 then decrement
i_down_done:
	sez						; set Z
	ret
